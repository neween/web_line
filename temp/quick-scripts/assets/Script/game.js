...
cc._RF.push(module, 'e5790GRLWhLHrOXPcH0KPCa', 'game', __filename);
// Script/game.js
...
cc.Class({
    ...
    createGrid: function createGrid(posx, posy, x, y) {
    ...
    onTouchCall: function onTouchCall(isfist, event) {
        ...
        cc.log("点到了格子为:", x, y);
        for (var i = 0; i < this.selectGridList.length; i++) {
            var element = this.selectGridList[i];
            if (element == cell) {
                return;
            }
        }
    ...
    adjustOnline: function adjustOnline() {
    ...
    cleanupMap: function cleanupMap() {
        for (var i = 0; i < this.gridList.length; i++) {
            for (var j = 1; j < this.gridList[i].length; j++) {
    ...
    swapCell: function swapCell(pos1, pos2) {
    ...
    addCellIntoMap: function addCellIntoMap() {
        for (var i = 0; i < this.gridList.length; i++) {
            for (var j = 1; j < this.gridList[i].length; j++) {
                var node = this.gridList[i][j];
                var dead = node.getComponent('grid')._dead;
    ...
    onDrawLine: function onDrawLine(end) {
